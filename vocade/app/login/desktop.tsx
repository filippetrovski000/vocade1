      ...
      const { error: signInError } = await supabase.auth.signInWithOtp({
        ...
        options: {
          emailRedirectTo: process.env.NODE_ENV === 'development'
            ? 'http://localhost:3000/auth-success?source=desktop'
            : 'https://vocade.vercel.app/auth-success?source=desktop',
        },
      });
      ...
      const { data } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          redirectTo: process.env.NODE_ENV === 'development'
            ? 'http://localhost:3000/auth/callback?source=desktop'
            : 'https://vocade.vercel.app/auth/callback?source=desktop',
          queryParams: {
            access_type: 'offline',
            prompt: 'consent'
          },
